...
void gemm_avx_block(float *A, float *B, float *C)
{
    for (int i = 0; i < N; i += BLOCKSIZE)
    {
        ...
        {
            __m128 Vec[BLOCKSIZE], a[BLOCKSIZE], b[BLOCKSIZE], temp;
            float ans;
            ...
            for(int k = 0; k < N; k+= BLOCKSIZE)
            {
                ...
                for(int m = 0; m < BLOCKSIZE; m++)
                {
                    a[m] = _mm_loadu_ps(&A[(i + m) * N + k]);
                    b[m] = _mm_loadu_ps(&B[(j + m) * N + k]);
                }
            ...
            _mm_storeu_ps(C + (i + 0) * N + j, Vec[0]);
            _mm_storeu_ps(C + (i + 1) * N + j, Vec[1]);
            _mm_storeu_ps(C + (i + 2) * N + j, Vec[2]);
            _mm_storeu_ps(C + (i + 3) * N + j, Vec[3]);
        }
