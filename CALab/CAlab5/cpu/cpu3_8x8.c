...
void gemm_avx_block(float *A, float *B, float *C)
{
    for (int i = 0; i < N; i += BLOCKSIZE)
    {
        ...
        {
            __m256 Vec[BLOCKSIZE], a[BLOCKSIZE], b[BLOCKSIZE], temp;
            float ans;
            for(int m = 0; m < BLOCKSIZE; m++)
            {
                Vec[m] = _mm256_setzero_ps();
            }
            for(int k = 0; k < N; k += BLOCKSIZE)
            { 
                for(int m = 0; m < BLOCKSIZE; m++)
                {
                    a[m] = _mm256_loadu_ps(&A[(i + m) * N + k]);
                    b[m] = _mm256_loadu_ps(&B[(j + m) * N + k]);
                }
            ...
            {
                _mm256_storeu_ps(C + (i + k) * N + j, Vec[k]);
            }
        }
    }
}
